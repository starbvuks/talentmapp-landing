...
export default function ProgressSlider({ items }) {
  ...
  return (
    <div className="w-full text-center">
      {/* Item image */}
      <div className="transition-all duration-150 delay-300 ease-in-out p-0">
        <div className="relative flex flex-col" ref={itemsRef}>
          {items.map((item, index) => (
            ...
            >
              <div className='w-full flex flex-col items-center justify-center h-[30em] bg-[#ede7f9]'>
                <img src={item.img} alt="logo" className='max-w-52' />
                <span className='text-4xl font-semibold mt-2'>{item.desc}</span>
                <span className='text-2xl w-2/3 mt-10'>{item.content}</span>
              </div>
            </Transition>
          ))}
      ...
      {/* Buttons */}
      <div className="self-center max-w-xs sm:max-w-sm md:max-w-3xl mx-auto grid grid-cols-2 gap-4 mt-8">
        {items.map((item, index) => (
          <button
            key={index}
          ...
          >
            <span className={`text-center flex flex-col items-center ${active === index ? '' : 'opacity-50 group-hover:opacity-100 group-focus:opacity-100 transition-opacity'}`}>
              <span className="flex items-center justify-center relative w-9 h-9 rounded-full bg-indigo-100 mb-2">
                <Image src={item.buttonIcon} alt={item.desc} width={32} height={32} />
              </span>
              ...
              <span className="block relative w-full bg-slate-200 h-1 rounded-full" role="progressbar" aria-valuenow={active === index ? progress : 0}>
                <span className="absolute inset-0 bg-[#5013AF] rounded-[inherit]" style={{ width: active === index ? `${progress}%` : '0%' }}></span>
              </span>
            </span>
          </button>
        ))}
